...
namespace MyPortfolio.Controllers
{
    public class ProjectController : Controller
    {
        ...
        {
            var categories = db.TblCategories.ToList(); // kategoriden gelen değerleri atadık burada 
            List<SelectListItem> categoryList = (from x in categories select new SelectListItem 
            {
                // burası seçilebilir değerleri gösteriyor
                Text = x.CategoryName,
                Value = x.CategoryId.ToString(),
            }).ToList();
            ViewBag.category = categoryList;  //ön yüze veri ataşımak için viewbag yazdık
